...
# adding a fourth organization to the network previously setup in the
# BYFN3 tutorial.
#
...
function printHelp () {
  echo "Usage: "
  echo "  eyfn4.sh up|down|restart|generate [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>]"
  echo "  eyfn4.sh -h|--help (print this message)"
  ...
  echo
  echo "	eyfn4.sh generate -c mychannel"
  echo "	eyfn4.sh up -c mychannel -s couchdb"
  echo "	eyfn4.sh up -l node"
  echo "	eyfn4.sh down -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	eyfn4.sh generate"
  echo "	eyfn4.sh up"
  echo "	eyfn4.sh down"
}
...
# be parked in the BYFN3 folder titled ``crypto-config``.
...
# If BYFN3 wasn't run abort
if [ ! -d crypto-config ]; then
  echo
  echo "ERROR: Please, run byfn3.sh first."
  echo
  exit 1
fi
